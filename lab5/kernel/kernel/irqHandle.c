...
void fileName(char*str,int index,char* filename);
int fatherDirPath(char* str, char* fatherPath);
...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		int type = sf->edx >> 3 == 1?DIRECTORY_TYPE:REGULAR_TYPE;
		if(destInode.type != type)
		{
			sf->eax = -1;
			return;
		}
		...
		int index = 0;
		char fatherPath[60],filename[60], dirName[60];		
		int type = sf->edx>>3==1?DIRECTORY_TYPE:REGULAR_TYPE;
		if ((sf->edx >> 3) % 2 == 0) { 
			//TODO: Open4        
			// 到了这里，目标文件不存在，并且CREATE位设置为1，并且要创建的目标文件是一个常规文件
			// Hint: readInode allocInode	
			index = fatherDirPath(str, fatherPath);		
			...
			// Hint: readInode allocInode
			index = fatherDirPath(str, fatherPath);
			fileName(str, index, dirName);
			readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, fatherPath);
			allocInode(&sBlock, gDesc,&fatherInode,fatherInodeOffset, &destInode,destInodeOffset,dirName,type);
...
void syscallWrite(struct StackFrame *sf) {
	...
	if(sf->ecx < 0 || sf->ecx >= MAX_DEV_NUM+MAX_FILE_NUM || file[sf->ecx-MAX_DEV_NUM].state == 0)
	{
		sf->eax = -1;
		return;
	}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	int firstSize = sBlock.blockSize-remainder;
	if(firstSize < sz)
	{
		int quo =  (sz-(sBlock.blockSize-remainder))/sBlock.blockSize;
		int rem = (sz-(sBlock.blockSize-remainder))%sBlock.blockSize;
		blockNum += rem == 0?quo:quo+1;
	}
	
	for(int i = 0; i < blockNum;++i)
	{
		readBlock(&sBlock,&inode,quotient+i, buffer+i*sBlock.blockSize);
	}
	MemCpy( str, buffer+remainder,sz);
	for(int i = 0; i < blockNum;++i)
		writeBlock(&sBlock,&inode,quotient+i, buffer+i*sBlock.blockSize);	
...
void syscallRemove(struct StackFrame *sf) {
	...
	int destInodeOffset = 0;
	int index = 0;
	char fatherPath[60],destfilename[60];
	...
	if (ret == 0) { // file exist
		...
		if (destInode.type != DIRECTORY_TYPE) {
			...
			index = fatherDirPath(str, fatherPath);
			fileName(str, index,destfilename);							
			readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, fatherPath);			
			freeInode(&sBlock, gDesc,&fatherInode, fatherInodeOffset,&destInode, destInodeOffset,destfilename, destInode.type);
			for(int i = 0; i < MAX_FILE_NUM; ++i)
...
int fatherDirPath(char* str, char* fatherPath)
{
	...
	stringChrR(str, '/', &index);
	stringCpy(str, fatherPath, index+1);	
	if(stringLen(str) < len)
		str[len-1] = '/';
	return index;
}
void fileName(char* str, int index,char* filename)
{
	int len = stringLen(str);
	stringCpy(str+index+1, filename, len-index-1);
